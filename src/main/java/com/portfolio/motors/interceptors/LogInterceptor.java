package com.portfolio.motors.interceptors;
...
public class LogInterceptor implements HandlerInterceptor {
   ...
   * 클라이언트(웹브라우저)의 요청을 컨트롤러에 전달 하기 전에 호출 된다.
  ...
  public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
    ...
    Parser usParser = new Parser();
    Client c = usParser.parse(ua);
    ...
    // URL에서 "?"이후에 전달되는 GET파라미터 문자열을 모두 모두 가져온다.
    ...
    for (String key : params.keySet()) {
      ...
      log.debug(String.format("(param) <-- %s = %s", key, String.join(",", value)));
    }

    // 5) 클라이언트가 머물렀던 이전 페이지와 이전 페이지에 머문 시간 확인하기
  ...
  public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler,
    ...
    // 2) 시작시간과 종료시간 사이의 차이를 구하면 페이지의 실행시간을 구할 수 있다.
    log.info(String.format("running time : %d(ms)\n", endTime - startTime));
